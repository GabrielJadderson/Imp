...
{
	type_check_DIRECTIVE_LIST(ast->directive_list, scope);
	type_check_BODY(ast->body, scope);
	return nullptr;
}
...
{
	if (FALCON_DEBUG_TYPECHECK)
		printf("|||||------> <directive_list> %d -- %d\n", directive_list->kind, phase);
	switch (directive_list->kind)
	{
		case DIRECTIVE_LIST::pair:
		{
			type_info_imp* ti_directive_list = type_check_DIRECTIVE_LIST(directive_list->val.pair.directive_list, scope);
			type_info_imp* ti_directive = type_check_DIRECTIVE(directive_list->val.pair.directive, scope);
		}break;
...
static void type_checker_print_type(type_info_imp* ti)
{
	if (ti && FALCON_DEBUG_TYPECHECK)
	{
		switch (ti->kind)
		{
			case type_info_imp::t_idk:
			{
				printf("%s", ti->type_id);
			}
			break;
			...
			case type_info_imp::t_boolK:
			{
				printf("bool");
			} break;
			case type_info_imp::t_stringK:
			{
				printf("string");
			} break;
			case type_info_imp::t_recordK:
			{
				printf("record of {");
				size_t length = ti->record_var_decl_list_types->length;
				for (size_t i = 0; i < length; i++)
				{
					type_info_imp* stored_ti = (type_info_imp*)array_at(ti->record_var_decl_list_types, i);
					if (stored_ti->kind != type_info_imp::t_recordK)
			...
			break;
			case type_info_imp::t_arrayK:
			{
				printf("array of ");
				type_checker_print_type(ti->array_type_info);
			}
			break;
			case type_info_imp::t_nullK:
			{
				printf("null");
			}
			break;
			case type_info_imp::t_typeK:
			{
				printf("%s", ti->type_id);
			}
			break;
...
static void type_checker_print_type_error(type_info_imp * ti)
{
	if (ti)
	{
		switch (ti->kind)
...
static void type_checker_print_stm(int lineno, type_info_imp * ti, char* type)
{
...
static void type_checker_print_template(int lineno, type_info_imp * a, type_info_imp* b, char* format)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_function_type_mismatch(int lineno, char* name, type_info_imp* ti_head, type_info_imp* body)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_function_no_return_type(int lineno, char* name, type_info_imp* ti_head)
{
	printf("\x1b[37;1m");
...
static void type_checker_print_term_function_call_non_function(int lineno, type_info_imp* function)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_variable_access_illegal_type(int lineno, type_info_imp* var_recursive)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_variable_array_access_illegal_indexing_method(int lineno, type_info_imp* exp_recursive)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_function_call_invalid_parameter_types(int lineno, type_info_imp* ti_func, type_info_imp*  act_list)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
	...
	{
		type_info_imp* parameter = (type_info_imp*)array_at(ti_func->function_var_decl_list_types, i);
	...
	{
		type_info_imp* argument = (type_info_imp *)array_at(act_list->act_list_types, i);
...
static void type_checker_print_stm_assign(int lineno, type_info_imp * a, type_info_imp * b)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_stm_assign_to_type(int lineno, type_info_imp * ti_exp, char* id)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_stm_assign_array_mismatch_types(int lineno, type_info_imp* A, type_info_imp* B)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_variable_array_access_non_array_type(int lineno, type_info_imp* var_recursive)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_stm_assign_record_mismatch_elements(int lineno, type_info_imp* ti_element_in_var, type_info_imp* ti_element_in_exp, int i)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_stm_assign_record_mismatch_count(int lineno, array* A, array* B)
{
	...
	{
		type_info_imp* parameter = (type_info_imp *)array_at(A, i);
		type_checker_print_type_error(parameter);
	...
	{
		type_info_imp* argument = (type_info_imp *)array_at(B, i);
		type_checker_print_type_error(argument);
...
static void type_checker_print_abs(int lineno, type_info_imp * ti)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static void type_checker_print_negation(int lineno, type_info_imp * ti)
{
	fprintf(stderr, "\x1b[37;1m");
	fprintf(stderr, "%d: ", lineno); //print  lineno
	fprintf(stderr, "\x1b[0m"); //reset color.
...
static type_info_imp* type_check_recursively_lookup_id(char* id, SymbolTable* scope, int lineno)
{
	if (id)
	{
		...
		if (symbol != NULL)
		{
			...
			if (ti)
			{
				...
				if (ti->kind == type_info_imp::t_idk)
				{
				...
				else if (ti->kind == type_info_imp::t_typeK)
				{
					if (ti->type_info_child)
					{
						if (ti->type_info_child->type_id)
					...
					{
						log_internal_error(__FILE__, __func__, __LINE__, "type declaration must have a valid type_info_imp_child. possible failure in structual buildup during collection phase.");
					}
...
static type_info_imp* type_check_recursively_lookup_type_info_imp(type_info_imp* ti, SymbolTable* scope, int lineno)
{
	if (ti)
	{
		if (ti->type_id)
...
/*
static void type_checker_print_debug_ti(type_info_imp* ti, char* str, int lineno)
{
	if (ti)
	{
...
/* takes in a type_info_imp struct and creates a new type_info_imp struct and copies all fields from the host to the target. */
static type_info_imp* type_check_create_copy_of_type_info_imp(type_info_imp* master)
{
	if (master)
	{
		type_info_imp* out = (type_info_imp *)Calloc(1, sizeof(type_info_imp));
...
static type_info_imp* type_checker_lookup_once(char* str, SymbolTable* scope, int lineno)
{
	if (str)
	{
...
static type_info_imp* type_check_lookup_once_type_info_imp(type_info_imp* ti, SymbolTable* scope, int lineno)
{
	if (ti)
	{
		if (ti->type_id)
...
type_info_imp* type_check_STRING(STRING * s, SymbolTable * scope)
{
	...
	switch (s->kind)
	{
		case STRING::stringK:
		{
			if (FALCON_DEBUG_TYPECHECK)
				printf("%d <string> -> \"%s\"\n", s->lineno, s->str);
			type_info_imp* ti = (type_info_imp *)Calloc(1, sizeof(type_info_imp));
			ti->kind = type_info_imp::t_stringK;
			ti->sym_table = scope;
			s->type = ti;
		}
		break;
...
type_info_imp* type_check_FUNCTION(FUNCTION * f, SymbolTable * scope)
{
	...
	type_info_imp* ti_head = type_check_HEAD(f->func_head, f->scope, f);
	type_info_imp* body = type_check_BODY(f->func_body, f->scope);
	...
	if (phase == type_phase_calculateK || phase == type_phase_verifyK)
	{
		...
		type_info_imp* ti_recursive_lookup = type_check_recursively_lookup_id(f->func_head->function_id, f->scope->next, f->lineno);
...
type_info_imp* type_check_HEAD(HEAD * head, SymbolTable * scope, FUNCTION* f)
{
	...
	type_info_imp* type = type_check_TYPE(head->return_type, scope->next);
	type_info_imp* ti_par_decl_list = type_check_PAR_DECL_LIST(head->params, scope);

	if (phase == type_phase_collectK)
	{

		//we want to create a copy of the type. and then assign our own data to it.
		type_info_imp* function_type = type_check_create_copy_of_type_info_imp(type);
		...
		function_type->function = f;
		function_type->type_info_child = type; //our child is <type>
		if (ti_par_decl_list)
		{
			if (ti_par_decl_list->function_var_decl_list_types)
			{
	...
	if (phase == type_phase_calculateK || phase == type_phase_verifyK)
	{
		type_info_imp* type_info_imp_self = type_checker_lookup_once(head->function_id, scope->next, head->lineno);
		if (type_info_imp_self)
		{
			//type_checker_print_debug_ti(type_info_imp_self, "head type_info_imp self", head->lineno);
			if (!type_info_imp_self->is_verified)
			{
				if (type_info_imp_self->type_info_child)
				{
					type_info_imp* type_info_imp_child_recursive = type_check_recursively_lookup_id(type_info_imp_self->type_info_child->type_id, scope->next, head->lineno);
					if (type_info_imp_child_recursive)
					{

						type_info_imp* function_type = type_check_create_copy_of_type_info_imp(type_info_imp_child_recursive);
					...
					{
						type_info_imp* function_type = type_check_create_copy_of_type_info_imp(type_info_imp_self->type_info_child);
						function_type->is_verified = 1;
						function_type->type_id = head->function_id;
						function_type->func_id = head->function_id;
						function_type->is_a_variable = 0;
				...
				{
					type_info_imp* function_type = type_check_create_copy_of_type_info_imp(type_info_imp_self);
					function_type->is_verified = 1;
					function_type->type_id = head->function_id;
					function_type->func_id = head->function_id;
					function_type->is_a_variable = 0;
...
type_info_imp* type_check_TYPE(TYPE * type, SymbolTable * scope)
{
	...
	switch (type->kind)
	{
		...
		case TYPE::type_arrayK:
		{
			...
			{
				type_info_imp* ti = (type_info_imp *)Calloc(1, sizeof(type_info_imp));
				ti->kind = type_info_imp::t_arrayK;
		...
		case TYPE::type_recordK:
		{
			...
			{
				type_info_imp* ti = (type_info_imp *)Calloc(1, sizeof(type_info_imp));
				ti->kind = type_info_imp::t_recordK;
				...
				if (FALCON_DEBUG_TYPECHECK)
				{
					...
					for (size_t i = 0; i < length; i++)
					{
						type_info_imp* tii = (type_info_imp*)array_at(arr, i);
...
type_info_imp* type_check_PAR_DECL_LIST(PAR_DECL_LIST * par_decl_list, SymbolTable * scope)
{
	...
	switch (par_decl_list->kind)
	{
		case PAR_DECL_LIST::par_decl_list_var_decl_listK:
		{
			...
			{
				array* arr = array_new();
				type_info_imp* ti = type_check_VAR_DECL_LIST(par_decl_list->val.var_decl_list, scope, arr);
		...
		{
			type_info_imp* type = (type_info_imp*)Calloc(1, sizeof(type_info_imp));
			par_decl_list->type = type;
			array * arr = array_new();
			par_decl_list->type->function_var_decl_list_types = arr;
		}
		break;
...
type_info_imp* type_check_VAR_DECL_LIST(VAR_DECL_LIST * var_decl_list, SymbolTable * scope, array * arr)
{
	...
	switch (var_decl_list->kind)
	{
		case VAR_DECL_LIST::var_decl_list_pairK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti = type_check_VAR_TYPE(var_decl_list->val.pair.head, scope);
		...
		case VAR_DECL_LIST::var_decl_var_typeK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti = type_check_VAR_TYPE(var_decl_list->val.var_type, scope);
				var_decl_list->type = ti;
				array_push(arr, ti);
			}
...
type_info_imp* type_check_VAR_TYPE(VAR_TYPE * var_type, SymbolTable * scope)
{
	if (FALCON_DEBUG_TYPECHECK)
		printf("|||||------> <var_type> %d -- %d\n", 0, phase);
	type_info_imp* ti = type_check_TYPE(var_type->type, scope);
	if (phase == type_phase_collectK)
	{
		//we want to create a copy of the type. and then assign our own data to it.
		type_info_imp* var_type_ti = type_check_create_copy_of_type_info_imp(ti);
	...
	if (phase == type_phase_verifyK || phase == type_phase_calculateK)
	{
		...
		type_info_imp* type_info_imp_self = type_checker_lookup_once(var_type->id, scope, var_type->lineno);
		if (type_info_imp_self)
		{
			if (!type_info_imp_self->is_verified)
			{
				if (type_info_imp_self->type_info_child)
				{
					//type_info_imp* type_info_imp_child_recursive = type_checker_lookup_once(var_type->ti->type_info_imp_child->type_id, scope, var_type->lineno);
					type_info_imp* type_info_imp_child_recursive = type_check_recursively_lookup_id(var_type->ti->type_info_child->type_id, scope, var_type->lineno);
					if (type_info_imp_child_recursive)
					{
						//update the symbol table to relfect the change.
						var_type->ti = type_info_imp_child_recursive;
					...
					{
						var_type->ti = type_info_imp_self->type_info_child;
						var_type->ti->is_verified = 1;
						var_type->ti->is_a_variable = 1;
						var_type->ti->is_a_function = 0;
						var_type->ti->is_a_type = 0;
						var_type->ti->symbol = putSymbol(scope, var_type->id, var_type->ti);
						//type_checker_print_debug_ti(type_info_imp_self->type_info_imp_child, "type_info_imp_self->type_info_imp_child", 0);
					}
				}
				else
				{
					//type_checker_print_debug_ti(type_info_imp_self, "type_info_imp_self", var_type->lineno);
					var_type->ti = type_info_imp_self;
					var_type->ti->is_verified = 1;
					var_type->ti->is_a_variable = 1;
					var_type->ti->is_a_function = 0;
					var_type->ti->is_a_type = 0;
...
type_info_imp* type_check_BODY(BODY * body, SymbolTable * scope)
{
	...
	type_info_imp* ti = type_check_STM_LIST(body->stm_list, scope);
...
{
	if (FALCON_DEBUG_TYPECHECK)
		printf("|||||------> <body_decl_list> %d -- %d\n", body_decl_list->kind, phase);
	switch (body_decl_list->kind)
	{
		case BODY_DECL_LIST::body_decl_list_list:
		{
			type_info_imp* ti_decl_list = type_check_DECL_LIST(body_decl_list->decl_list, scope);
			return ti_decl_list;
		}break;
...
type_info_imp* type_check_DECL_LIST(DECL_LIST * decl_list, SymbolTable * scope)
{
	...
	switch (decl_list->kind)
	{
		...
		{
			type_check_DECL(decl_list->val.pair.head, scope);
			type_check_DECL_LIST(decl_list->val.pair.next, scope);
		}
		...
		{
			//type_info_imp* ti_decl = type_check_DECL(decl_list->val.decl, scope);
		}
		break;
		default:  break;
	}
...
type_info_imp* type_check_DECL(DECL * decl, SymbolTable * scope)
{
	...
	switch (decl->kind)
	{
		...
		{
			type_info_imp* ti = type_check_TYPE(decl->val.type, scope);

			if (phase == type_phase_collectK)
			{
				//type_checker_print_debug_ti(ti, "decl_decl_decl_ti", decl->lineno);
				//we want to create a copy of the type. and then assign our own data to it.
				type_info_imp* decl_type = type_check_create_copy_of_type_info_imp(ti);
				decl_type->kind = type_info_imp::t_typeK;
			...
			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
				type_info_imp* type_info_imp_self = type_checker_lookup_once(decl->decl_id, scope, decl->lineno);
				if (type_info_imp_self)
				{
					if (type_info_imp_self->type_info_child)
					{
						//dumpSymbolTable(scope);
						type_info_imp* type_info_imp_child_recursive = type_check_recursively_lookup_id(type_info_imp_self->type_info_child->type_id, scope, decl->lineno);
						if (type_info_imp_child_recursive)
						{
							if (!type_info_imp_self->is_verified)
							{
								//update the symbol table to relfect the change.
								decl->type = type_info_imp_child_recursive;
						...
						}
						{
							if (!type_info_imp_self->is_verified)
							{
								decl->type = type_info_imp_self->type_info_child;
								decl->type->is_verified = 1;
								decl->type->is_a_variable = 0;
								decl->type->is_a_function = 0;
								decl->type->is_a_type = 1;
		...
		case DECL::decl_functionK:
		{
			...
			type_info_imp* ti = type_check_FUNCTION(decl->val.function, scope);
			decl->type = ti;

			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
				type_info_imp* function_self_lookup = type_checker_lookup_once(decl->val.function->func_head->function_id, scope, decl->lineno);
		...
		case DECL::decl_varsK:
		{
			...
			{
				array* arr = array_new();
				type_info_imp* ti = type_check_VAR_DECL_LIST(decl->val.var_decl_list, scope, arr);
				decl->type = ti;
		...
		{
			type_info_imp* ti_var_type = type_check_VAR_TYPE(decl->var_type, scope);
			type_info_imp* ti_exp = type_check_EXP(decl->exp, scope);

			if (phase == type_phase_verifyK)
			{
				if (ti_var_type->kind == ti_exp->kind)
				{

				}
				else
				{
					type_checker_print_stm_assign(decl->lineno, ti_var_type, ti_exp);
				}
			}
		...
		{
			type_info_imp* ti_body = type_check_BODY(decl->body, scope);
		}break;
...
type_info_imp* type_check_STM_LIST(STM_LIST * stm_list, SymbolTable * scope)
{
	...
	switch (stm_list->kind)
	{
		...
		{
			type_info_imp* ti = type_check_STM(stm_list->val.pair.head, scope); //first element
			type_info_imp* ti_stm_list = type_check_STM_LIST(stm_list->val.pair.tail, scope); //next/last element
...
type_info_imp* type_check_STM(STM * stm, SymbolTable * scope)
{
	...
	switch (stm->kind)
	{
		case STM::stm_returnK: //only the return statement should change the type of stm and return a valid type.
		{
			type_info_imp* ti = type_check_EXP(stm->val.exp, scope);
			stm->type = ti;
			stm->type->has_return++;
		...
		case STM::stm_printf:
		{
			...
			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
				if (ti)
				{
					switch (ti->kind)
					{
						case type_info_imp::t_idk:break;
						case type_info_imp::t_intK:break;
						case type_info_imp::t_boolK:break;
						case type_info_imp::t_stringK:break;
						case type_info_imp::t_nullK:break;
						case type_info_imp::t_typeK:break;
						case type_info_imp::t_arrayK:
						{
							type_checker_print_stm(stm->lineno, ti, "write");
						}
						break;
						case type_info_imp::t_recordK: //don't allow
						{
							type_checker_print_stm(stm->lineno, ti, "write");
						}
						break;
		...
		{
			type_info_imp* ti = type_check_VARIABLE(stm->val.var, scope);
		...
		case STM::stm_allocate_lengthK: //TODO: refactor
		{
			//requirement: exp must be of type int.
			type_info_imp* ti_var = type_check_VARIABLE(stm->val.allocate_length.var, scope);
			type_info_imp* ti_exp = type_check_EXP(stm->val.allocate_length.exp, scope);
			stm->type = ti_var;
			if (ti_var)
			{
				switch (ti_var->kind)
		...
		{
			type_info_imp* ti_var = type_check_VARIABLE(stm->val.assignment.var, scope);
			type_info_imp* ti_exp = type_check_EXP(stm->val.assignment.exp, scope);
			...
			{
				//log_warning("car 2");
				//type_info_imp * var_recursive_lookup = type_check_recursively_lookup_type_info_imp(ti_var, scope, stm->lineno, __LINE__);
				//type_info_imp * exp_recursive_lookup = type_check_recursively_lookup_type_info_imp(ti_exp, scope, stm->lineno, __LINE__);
				type_info_imp * var_recursive_lookup = ti_var;
				type_info_imp * exp_recursive_lookup = ti_exp;
				...
				if (var_recursive_lookup->kind == exp_recursive_lookup->kind)
				{
					...
					if (var_recursive_lookup->kind == type_info_imp::t_recordK && exp_recursive_lookup->kind == type_info_imp::t_recordK)
					{
						...
						if (var_record_length == exp_record_length)
						{
							...
							{
								type_info_imp* var_record_element = (type_info_imp*)array_at(var_recursive_lookup->record_var_decl_list_types, i);
								type_info_imp* exp_record_element = (type_info_imp*)array_at(exp_recursive_lookup->record_var_decl_list_types, i);

								type_info_imp* var_record_element_lookup = type_check_recursively_lookup_type_info_imp(
									var_record_element, var_recursive_lookup->record_scope, stm->lineno);

								type_info_imp* exp_record_element_lookup = type_check_recursively_lookup_type_info_imp(
							...
							if (found_error == 0)
							{
								type_info_imp* result = type_check_create_copy_of_type_info_imp(exp_recursive_lookup);
								result->is_a_type = 0;
					...
					else if (var_recursive_lookup->kind == type_info_imp::t_arrayK && exp_recursive_lookup->kind == type_info_imp::t_arrayK) //if assigning two arrays make sure their type is also the same.
					{
				...
				else
				{
					//type_checker_print_debug_ti(var_recursive_lookup, "var_recursive_lookup", stm->lineno);
					//type_checker_print_debug_ti(exp_recursive_lookup, "exp_recursive_lookup", stm->lineno);
					if (var_recursive_lookup->kind == type_info_imp::t_boolK)
					{
						if (exp_recursive_lookup->kind == type_info_imp::t_intK)
					...
					else if (var_recursive_lookup->kind == type_info_imp::t_intK)
					{
						if (exp_recursive_lookup->kind == type_info_imp::t_boolK)
						{
							type_info_imp* result = type_check_create_copy_of_type_info_imp(exp_recursive_lookup);
							result->is_a_type = 0;
							result->is_a_function = 0;
					...
					else if (var_recursive_lookup->kind == type_info_imp::t_stringK)
					{
						if (exp_recursive_lookup->kind != type_info_imp::t_stringK)
							type_checker_print_stm_assign(stm->lineno, var_recursive_lookup, exp_recursive_lookup);
					}
					else if (var_recursive_lookup->kind == type_info_imp::t_recordK)
					{
						if (exp_recursive_lookup->kind == type_info_imp::t_nullK)
						{
							type_info_imp* result = type_check_create_copy_of_type_info_imp(exp_recursive_lookup);
							result->is_a_type = 0;
							result->is_a_function = 0;
					...
					else if (var_recursive_lookup->kind == type_info_imp::t_arrayK)
					{
						if (exp_recursive_lookup->kind == type_info_imp::t_nullK)
						{
							type_info_imp* result = type_check_create_copy_of_type_info_imp(exp_recursive_lookup);
					...
					else if (var_recursive_lookup->kind == type_info_imp::t_nullK)
					{
						if (exp_recursive_lookup->kind == type_info_imp::t_arrayK)
						{
							type_info_imp* result = type_check_create_copy_of_type_info_imp(exp_recursive_lookup);
							result->is_a_type = 0;
							result->is_a_function = 0;
						...
						else if (exp_recursive_lookup->kind == type_info_imp::t_recordK)
						{
							type_info_imp* result = type_check_create_copy_of_type_info_imp(exp_recursive_lookup);
							result->is_a_type = 0;
							result->is_a_function = 0;
		...
		case STM::stm_ifK:
		{
			...
			type_info_imp* ti_exp = type_check_EXP(stm->val.if_stm.exp, scope);
			//stm->type = ti_exp;
			array* arr = array_new();
			type_info_imp* ti_stm = type_check_STM(stm->val.if_stm.stm, scope);
			...
			if (phase == type_phase_verifyK)
			{
				if (ti_exp)
				{
					if (ti_exp->kind == type_info_imp::t_idk) {}
					else if (ti_exp->kind == type_info_imp::t_intK) {}
					else if (ti_exp->kind == type_info_imp::t_boolK) {}
					else
					{
		...
		{
			//requirement: exp must be of type bool.
			type_info_imp* ti_exp = type_check_EXP(stm->val.if_else_stm.exp, scope);
				//stm->type = ti_exp;
			array* arr = array_new();
			type_info_imp* ti_stm1 = type_check_STM(stm->val.if_else_stm.stm_then, scope);
			type_info_imp* ti_stm2 = type_check_STM(stm->val.if_else_stm.stm_else, scope);
		...
		case STM::stm_whileK:
		{
			type_info_imp* ti_exp = type_check_EXP(stm->val.while_stm.exp, scope);

			array* arr = array_new();
			type_info_imp* ti_stm = type_check_STM(stm->val.while_stm.stm, scope);
			array_push(arr, ti_stm);
			stm->type = ti_stm;
			stm->type->return_type = ti_stm->return_type;
		...
		{
				//TODO: check the type.
			type_info_imp* ti_var_type = type_check_VAR_TYPE(stm->val.for_stm.var_type, scope);
			type_info_imp* ti_exp = type_check_EXP(stm->val.for_stm.exp, scope);
			type_info_imp* ti_stm = type_check_STM(stm->val.for_stm.stm, scope);
			type_info_imp* ti_body = type_check_BODY(stm->val.for_stm.body, scope);
		}
		break;
		...
		case STM::stm_listK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti = type_check_STM_LIST(stm->val.list, scope);
				if (ti != NULL)
				{
					stm->type = ti;
					return ti;
...
type_info_imp* type_check_VARIABLE(VARIABLE * var, SymbolTable * scope)
{
	...
	switch (var->kind)
	{
		case VARIABLE::variable_idK:
		{
			...
			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
				type_info_imp* ti_lookup_once = type_checker_lookup_once(var->val.id, scope, var->lineno);

				if (!var->type->is_verified)
				{
					if (ti_lookup_once)
					{
						type_info_imp* result = type_check_create_copy_of_type_info_imp(ti_lookup_once);
		...
		{
			type_info_imp* ti_var = type_check_VARIABLE(var->val.array_access.var, scope);
			type_info_imp* ti_exp = type_check_EXP(var->val.array_access.exp, scope);
			...
			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
				...
				//type_checker_print_debug_ti(ti_var->array_type_info_imp, "var.array.access->array_type_info_imp", var->lineno);
				//log_info("scope");
				//dumpSymbolTable(scope);
				//log_info("sym_table");
				...
				type_info_imp* var_recursive = type_check_recursively_lookup_type_info_imp(ti_var, ti_var->sym_table, var->lineno);

				//type_checker_print_debug_ti(var_recursive, "var_recusive", var->lineno);

				type_info_imp* exp_recursive = type_check_recursively_lookup_type_info_imp(ti_exp, scope, var->lineno);
				//type_checker_print_debug_ti(exp_recursive, "exp_recusive", var->lineno);

				if (exp_recursive->kind == type_info_imp::t_intK)
				{
				...
				if (var_recursive->kind == type_info_imp::t_arrayK)
				{
					//GHJ: it is important here not to use our own scope, but the scope of the array type itself.
					type_info_imp* array_type_recursive_lookup = type_check_lookup_once_type_info_imp(var_recursive->array_type_info, var_recursive->sym_table, var->lineno);
					if (array_type_recursive_lookup)
					{
						//type_checker_print_debug_ti(array_type_recursive_lookup, "array_type_info_imp", var->lineno);
						var->type = array_type_recursive_lookup;
					}
				}
				else if (var_recursive->kind == type_info_imp::t_nullK)
				{
		...
		case VARIABLE::variable_access_idK:
		{
			...
			type_info_imp* ti_var = type_check_VARIABLE(var->val.access.var, scope);
			...
			if (phase == type_phase_calculateK || phase == type_phase_verifyK) //TODO: FIX AND INTEGRATE ERROR MESSAGES.
			{
				...
				if (var_recursive->kind == type_info_imp::t_recordK)
				{
					//GHJ: if it's a valid record, then go inside and lookup the child id in the record scope.
					type_info_imp* child_recursive_id = type_check_recursively_lookup_id(var->val.access.id, var_recursive->record_scope, var->lineno);
					if (child_recursive_id)
					{
						type_info_imp* child_recursive_type = type_check_recursively_lookup_type_info_imp(child_recursive_id, var_recursive->record_scope, var->lineno);
						type_info_imp* result = type_check_create_copy_of_type_info_imp(child_recursive_type);
						result->is_a_type = 0;
						result->is_a_function = 0;
...
type_info_imp* type_check_EXP(EXP * exp, SymbolTable * scope)
{
	...
	switch (exp->kind)
	{
		case EXP::exp_timesK:
		{
			type_info_imp* a = type_check_EXP(exp->val.op.left, scope);
			type_info_imp* b = type_check_EXP(exp->val.op.right, scope);
			exp->type = b;
			if (a && b)
			{
				if (phase == type_phase_calculateK)
				{
					...
					if (a->kind == type_info_imp::t_idk)
					{
						//a * b -> a = id, -> id * id = legal -> id * 5 = legal
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_intK) {}
						else type_checker_print_template(exp->lineno, a, b, "Multiplication is illegal for");
					}
					else if (a->kind == type_info_imp::t_intK) //reverse order applies.
					{
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_intK) {}
						else type_checker_print_template(exp->lineno, a, b, "Multiplication is illegal for");
		...
		case EXP::exp_divK:
		{
			...
			if (a && b)
			{
				if (phase == type_phase_calculateK)
				{
					...
					else if (a->kind == type_info_imp::t_intK)
					{
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_intK) {}
						else type_checker_print_template(exp->lineno, a, b, "Division is illegal for");
			...
			if (a && b)
			{
				if (phase == type_phase_calculateK)
				{
					...
					if (a->kind == type_info_imp::t_idk)
					{
						...
						else if (b->kind == type_info_imp::t_stringK) {}
					...
					else if (a->kind == type_info_imp::t_stringK)
					{
			...
			if (a && b)
			{
				...
				if (phase == type_phase_calculateK)
				{
					...
					if (a->kind == type_info_imp::t_idk)
					{
						...
						else if (b->kind == type_info_imp::t_boolK) {}
						else if (b->kind == type_info_imp::t_stringK) {}
						else if (b->kind == type_info_imp::t_recordK) {}
						else if (b->kind == type_info_imp::t_arrayK) {}
					...
					else if (a->kind == type_info_imp::t_boolK)
					{
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_intK) {}
						else if (b->kind == type_info_imp::t_boolK) {}
					...
					else if (a->kind == type_info_imp::t_recordK)
					{
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_recordK) {}
						else if (b->kind == type_info_imp::t_nullK) {}
						else type_checker_print_template(exp->lineno, a, b, "Equality is illegal for");
					}
					else if (a->kind == type_info_imp::t_nullK)
					{
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_recordK) {}
						else if (b->kind == type_info_imp::t_arrayK) {}
					...
					else if (a->kind == type_info_imp::t_arrayK)
					{
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_arrayK) {}
						else if (b->kind == type_info_imp::t_nullK) {}
		...
		case EXP::exp_greater_equalK:
		{
			...
			if (a && b)
			{
				if (phase == type_phase_calculateK)
				{
					...
					if (a->kind == type_info_imp::t_idk) //TODO: check if this is really triggered, since variable is recursive now.
					{
						//a - b -> a = id, -> id - id = legal -> id - 5 = legal
						if (b->kind == type_info_imp::t_idk) {}
						else if (b->kind == type_info_imp::t_intK) {}
		...
		case EXP::exp_termK:
		{
			type_info_imp* ti = type_check_TERM(exp->val.term, scope);
			exp->type = ti;
		}
		break;
...
type_info_imp* type_check_TERM(TERM * term, SymbolTable * scope)
{
	...
	switch (term->kind)
	{
		case TERM::term_varK:
		{
			type_info_imp* ti = type_check_VARIABLE(term->val.var, scope);
			term->type = ti;
			return term->type;
		}
		break;
		...
		case TERM::term_id_act_listK:
		{
			...
			{
				type_info_imp* act_list = type_check_ACT_LIST(term->val.id_act_list.act_list, scope, NULL);
				if (act_list)
				{
			...
			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
				...
				type_info_imp* function = type_checker_lookup_once(term->val.id_act_list.id, scope, term->lineno);
				if (function)
				{
					if (function->is_a_function)
					{
						...
						array* arr = array_new();
						type_info_imp* act_list = type_check_ACT_LIST(term->val.id_act_list.act_list, scope, arr);

						act_list->act_list_types = arr;
						term->val.id_act_list.act_list->type->act_list_types = arr;
						...
						if (function->function_var_decl_list_types)
						{
							...
							else //else the parameter count is correct. then each parameter type is checked.
							{
								...
								{
									type_info_imp* parameter = (type_info_imp*)array_at(function->function_var_decl_list_types, i);
									//GHJ: just to make sure it's correct, let's look up the parameter using the function scope.
									type_info_imp* parameter_lookup = type_check_recursively_lookup_type_info_imp(parameter, function->sym_table, term->lineno);
									//type_checker_print_debug_ti(parameter_lookup, "parameter_lookup", term->lineno);

									type_info_imp* argument = (type_info_imp*)array_at(arr, i);
									...
									if (parameter)
									{
										if (argument)
										{
											...
											{
												if (parameter_lookup->kind == type_info_imp::t_arrayK)
												{
													if (argument->kind == type_info_imp::t_nullK) continue;
													else found_error = 1;
												}
												else if (parameter->kind == type_info_imp::t_recordK)
												{
													if (argument->kind == type_info_imp::t_nullK) continue;
													else found_error = 1;
												}
												else if (argument->kind == type_info_imp::t_arrayK)
												{
													if (parameter->kind == type_info_imp::t_nullK) continue;
													else found_error = 1;
												}
												else if (argument->kind == type_info_imp::t_recordK)
												{
													if (parameter->kind == type_info_imp::t_nullK) continue;
													else found_error = 1;
												}
		...
		{
			type_info_imp* ti = type_check_EXP(term->val.exp, scope);
			ti->sym_table = scope;
			term->type = ti;
		}
		break;
		...
		{
			type_info_imp* ti = type_check_TERM(term->val.term, scope);
			ti->sym_table = scope;
			term->type = ti;
		...
		case TERM::term_nullK: //null type
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti = (type_info_imp*)Calloc(1, sizeof(type_info_imp));
				ti->kind = type_info_imp::t_nullK;
				ti->sym_table = scope;
				term->type = ti;
				return term->type;
			}
		...
		case TERM::term_stringK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti = type_check_STRING(term->val.string, scope);
				ti->sym_table = scope;
				term->type = ti;
...
type_info_imp* type_check_ACT_LIST(ACT_LIST * act_list, SymbolTable * scope, array * arr)
{
	...
	switch (act_list->kind)
	{
		case ACT_LIST::act_list_exp_listK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti = type_check_EXP_LIST(act_list->exp_list, scope, NULL);
				if (ti)
				{
			...
			if (phase == type_phase_verifyK || phase == type_phase_calculateK)
			{
				type_info_imp* ti = type_check_EXP_LIST(act_list->exp_list, scope, arr);
				act_list->type = ti;
...
type_info_imp* type_check_EXP_LIST(EXP_LIST * exp_list, SymbolTable * scope, array * arr)
{
	...
	switch (exp_list->kind)
	{
		case EXP_LIST::exp_list_pairK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti_exp = type_check_EXP(exp_list->val.pair.head, scope);
			...
			if (phase == type_phase_verifyK || phase == type_phase_calculateK)
			{
				...
				if (phase == type_phase_verifyK)
					exp_list->type->act_list_count++;
				type_info_imp* ti_exp_list = type_check_EXP_LIST(exp_list->val.pair.tail, scope, arr);
		...
		case EXP_LIST::exp_list_expressionK:
		{
			if (phase == type_phase_collectK)
			{
				type_info_imp* ti_exp = type_check_EXP(exp_list->val.exp, scope);
				exp_list->type = ti_exp;
			}
			if (phase == type_phase_calculateK || phase == type_phase_verifyK)
			{
