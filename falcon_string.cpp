...
{
	size_t len = strlen(array);
	char* str_3 = (char*)Calloc(len - offset + 1, sizeof(char));
...
{
	char* buffer = (char*)Calloc(21, sizeof(char)); //64-bit integer has max 20 characters + 1 for null.
...
string string_new_memcpy(char* text)
{
	...
	if (text)
	{
		...
		str.Text = (char*)Calloc((str.Length + 1), sizeof(char));
...
{
	string_builder* sb = (string_builder *)Calloc(1, sizeof(string_builder));
