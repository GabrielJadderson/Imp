...
%union {
   ...
   struct BODY* body;
   struct BODY_DECL_LIST* body_decl_list;
...
%token <dynamic_string> tCOMMAND_LOAD
%token tEQUALITY
%token tLESSER_EQUAL
...
%type <body_decl_list> body_decl_list
...
directive : tCOMMAND_LOAD
	{
		$$ = make_DIRECTIVE_load($1);
	}
	| %empty
	{
	}
...
function : head '{' body '}'
	{
		$$ = make_FUNC($1, $3, make_TAIL(""));
	}
;


head : tFUNC tIDENTIFIER '(' par_decl_list ')' tRIGHT_ARROW type
	{
		$$ = make_HEAD($2, $4, $7, modifier_type, visibility_type);
...
body : body_decl_list statement_list
	{
		$$ = make_BODY($1, $2);
	}
;
...
body_decl_list : decl_list
	{
		$$ = make_BODY_DECL_LIST_list($1);
	}
	| %empty
	{
		$$ = make_BODY_DECL_LIST_empty();
	}
;
