...
{
	falcon_config* config = (falcon_config*)Calloc(1, sizeof(falcon_config));
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		{
			config->FALCON_DEBUG_IR = 1;
		}
		if (strcmp(argv[i], "-dw") == 0)
		{
			config->FALCON_DEBUG_WEEDER = 1;
		}
		if (strcmp(argv[i], "-dirw") == 0)
		{
			config->FALCON_DEBUG_IR_WEED = 1;
		}
	}
